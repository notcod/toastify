...
import { $, component$, useComputed$, useOnDocument, useSignal, useStore, useStyles$, useTask$, useOnWindow, sync$ } from '@builder.io/qwik';
import type { Theme, ToastT, ToastToDismiss, ToasterProps, ToasterStore } from '../utils/types';
import { Toast } from './toast-card';
import { GAP, TOAST_LIFETIME, TOAST_WIDTH, VIEWPORT_OFFSET, VISIBLE_TOASTS_AMOUNT } from '../constants';
...
export const Toaster = component$<ToasterProps>((props) => {
    ...
    $((ev: CustomEvent) => {
      ...
      toastState.subscribe((toast) => {
        if ((toast as ToastToDismiss).dismiss) {
          state.toasts = state.toasts.map((t) => (t.id === toast.id ? { ...t, delete: true } : t));
          return;
        }

        const indexOfExistingToast = state.toasts.findIndex((t) => t.id === toast.id);

        // Update the toast if it already exists
        if (indexOfExistingToast !== -1) {
          state.toasts = [...state.toasts.slice(0, indexOfExistingToast), { ...state.toasts[indexOfExistingToast], ...toast }, ...state.toasts.slice(indexOfExistingToast + 1)];
          return;
        }
  ...
  const possiblePositions = useComputed$(() => {
    return Array.from(new Set([props.position ?? position].concat(state.toasts.filter((toast) => toast.position!).map((toast) => toast.position!))));
  });
  ...
  const removeToast = $((toast: ToastT) => (state.toasts = state.toasts.filter(({ id }) => id !== toast.id)));

  useOnWindow(
    'DOMContentLoaded',
    sync$((e: Event) => {
      const documentEl = e.target as Document;
      const list = documentEl.getElementById('qtoastify-toaster');
      ...
      const themeFromMedia = window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
  ...
  const onBlurHandler = $((event: FocusEvent, element: HTMLOListElement) => {
    if (isFocusWithinRef.value && !element.contains(event.relatedTarget as Node)) {
  ...
  const onPointerDownHandler = $((_: PointerEvent, element: HTMLOListElement) => {
    const isNotDismissible = element.dataset.dismissible === 'false';
      ...
      {possiblePositions.value.map((position, index) => {
        ...
        return (
          <ol
            ...
            onPointerUp$={[$(() => (state.interacting = false)), props.onPointerUp$]}
          >
            {state.toasts
