...
export const Loader = component$(({ visible }: { visible: boolean }) => {
  return (
    <div class="qtoastify-loading-wrapper" data-visible={visible}>
      <div class="qtoastify-spinner">
        {bars.map((_, i) => (
          <div class="qtoastify-loading-bar" key={`spinner-bar-${i}`} />
        ))}
      </div>
    </div>
  );
